...
tags$p("This report pulls data specifically for CRO analysis. All data is collected from Google Analytics and can be exported into an XLSX workbook.",
       ...
       br(),
       br(),
       "Follow the steps to generate a report.",
       br(),
       br()
       )
```
...
tags$br()
tags$br()
tags$h4(em("2. Wait for account details to load. This could take a while. If you update any account details please wait for tables and graphs to load. ( We will be instaling a loader at some point )"))
```
</br>
```{r date, echo=FALSE}
renderUI({
